...
lazy val mdocSettings = Seq(
  ...
  unidocProjectFilter in (ScalaUnidoc, unidoc) := inProjects(monixKafka),
  target in (ScalaUnidoc, unidoc) := (baseDirectory in LocalRootProject).value / "website" / "p" / "api",
  ...
  scalacOptions in (ScalaUnidoc, unidoc) ++= Seq(
    "-doc-source-url", s"https://github.com/monix/monix-kafka/tree/v${version.value}€{FILE_PATH}.scala",
    "-sourcepath", baseDirectory.in(LocalRootProject).value.getAbsolutePath,
    "-doc-title", "Monix Kafka",
...
updateSiteVariables in ThisBuild := {
  ...
  val variables =
    Map[String, String](
      ...
      "coreModuleName" -> (moduleName in monixKafka).value,
      "latestVersion" -> version.value,
      "scalaPublishVersions" -> {
        val minorVersions = (crossScalaVersions in monixKafka).value.map(minorVersion)
