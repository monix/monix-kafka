...
class MonixKafkaTest extends FunSuite {
  ...
  test("merge by commit callback when the commit callbacks is different") {
    val count = 10000
    val topicName = "monix-kafka-manual-commit-tests"

    val producer = KafkaProducerSink[String, String](producerCfg, io)

    val consumerCfg1 = KafkaConsumerConfig.default.copy(
      bootstrapServers = List("127.0.0.1:6001"),
      groupId = "kafka-tests-1",
      autoOffsetReset = AutoOffsetReset.Earliest
    )

    val consumerCfg2 = KafkaConsumerConfig.default.copy(
      bootstrapServers = List("127.0.0.1:6001"),
      groupId = "kafka-tests-2",
      autoOffsetReset = AutoOffsetReset.Earliest
    )

    val consumer1 = KafkaConsumerObservable.manualCommit[String, String](consumerCfg1, List(topicName))
    val consumer2 = KafkaConsumerObservable.manualCommit[String, String](consumerCfg2, List(topicName))
    ...
    val observable1: Observable[CommittableOffset] = consumer1
      .executeOn(io)
      .map(_.committableOffset)
    val observable2: Observable[CommittableOffset] = consumer2
      .executeOn(io)
      .map(_.committableOffset)
    val listT = Observable
      .from(Seq(observable1, observable2))
      .merge
      .bufferTumbling(count)
