...
case class KafkaStreamsConfig(
  ...
  def toMap: Map[String,String] = Map(
    ...
    "reconnect.backoff.max" -> reconnectBackoffMax.toMillis.toString,
    ...
    "state.cleanup.delay" -> stateCleanupDelay.toMillis.toString,
    "state.dir" -> stateDir,
...
object KafkaProducerConfig {
  ...
  def apply(source: Config, includeDefaults: Boolean = true): KafkaStreamsConfig = {
    ...
    KafkaStreamsConfig(
      ...
      cacheMaxBytesBuffering = config.getInt("cache.max.bytes.buffering"),
      ...
      reconnectBackoffMax = config.getInt("reconnect.backoff.max").millis,
      ...
      rocksdbConfigSetter = config.getString("rocksdb.config.setter"),
      securityProtocol = SecurityProtocol(config.getString("security.protocol")),
      sendBufferBytes = config.getInt("send.buffer.bytes"),
      stateCleanupDelay = config.getInt("state.cleanup.delay").millis,
